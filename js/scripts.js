...
function checkFailDiceRoll(roll){
...
function computerRoundEval (){
  if (roundScore > 18) {
    console.log("We're doing pretty well, let's cash out");
    return false;
  }else if (roundScore < 18) {
...
function compTurn() {
  ...
  while (computerRoller === true) {
    ...
    computerRoller = checkFailDiceRoll(diceRollResult);
    console.log(computerRoller);
    computerTurns++;
...
function endTurn(){
  ...
  function pigWinnerReset() {
    currentRoster.players.forEach(function(x) {
      x.isComputer = false;
    });
  }
